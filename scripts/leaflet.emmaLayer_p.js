				...
				if(Math.pow(e_center.x - center.x,2)+Math.pow(e_center.y - center.y,2) < Math.pow(radius+e_radius,2)){//円が衝突
					while((Math.pow(e_center.x - center.x,2)+Math.pow(e_center.y - center.y,2) < Math.pow(radius+e_radius,2)) && (e_radius > 0)){//円が衝突してるうちは
						if ($("#collision_focus_glue").val() == "all_together"){
							...
							var maxDistance = 8*parseInt(self._focusGlueLayer.style.width)/2; // ズームアウトする距離のしきい値を設定	5*parseInt(self._focusGlueLayer.style.width)/2
...
//追加
		if(init){
			// 複数のマーカーを設定
			var markers = [
				{ lat: 35.164889, lng: 136.899701, comment: "指定地点２" },
				{ lat: 35.156311, lng: 136.923079, comment: "指定地点１" },
				{ lat: 35.167373, lng: 136.908826, comment: "指定地点３" }
			];
