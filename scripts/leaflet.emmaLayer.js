...
/*							// マウスカーソルの座標を取得
							...
							// カーソル位置を地図の緯度経度に変換
							var cursorLatLng = self._contextMap.containerPointToLatLng([cursorPosition.x, cursorPosition.y]);


/*							// 点を描画する関数
							const drawPoint = (map, latLng, color) => {
							...
							// マウスカーソル位置に点を描画
							drawPoint(self._contextMap, cursorLatLng, "red");*/


							// カーソル位置を地図の中心に設定し、ズームイン
							//self._contextMap.setView([cursorLatLng.lat, cursorLatLng.lng], self._contextMap.getZoom() + 1); // ズームインも一緒に行う
							
							
/*							// ズームイン後に地図の中心に点を描画
							const centerLatLng = self._contextMap.getCenter();
							drawPoint(self._contextMap, centerLatLng, "blue");*/
							...
							// Focusの中心を取得 (現在の位置)
							var focusCenter = {
								x: parseInt(self._focusGlueLayer.style.left) + parseInt(self._focusGlueLayer.style.width) / 2,
								y: parseInt(self._focusGlueLayer.style.top) + parseInt(self._focusGlueLayer.style.height) / 2
							};

							// ズレを徐々に補正するためのステップ数と間隔
							var stepCount = 50; // 補正を分割するステップ数
							var step = 0; // 現在のステップ
							var interval = 20; // 更新間隔 (ms)
							var xDiff = (cursorPosition.x - focusCenter.x) / stepCount; // X方向の移動量
							var yDiff = (cursorPosition.y - focusCenter.y) / stepCount; // Y方向の移動量

							// 補正処理のタイマー
							var intervalId = setInterval(function() {
								if (step >= stepCount) {
									clearInterval(intervalId); // 全ステップ完了時に停止
									return;
								}
								...
								// Focusの位置を更新
								self._focusGlueLayer.style.left = (focusCenter.x - parseInt(self._focusGlueLayer.style.width) / 2) + "px";
								self._focusGlueLayer.style.top = (focusCenter.y - parseInt(self._focusGlueLayer.style.height) / 2) + "px";
							...
							//e._focusMap.flyTo([e._focusMap.getCenter().lat, e._focusMap.getCenter().lng], e._focusMap.getZoom()); // ズームインも一緒に行う
							...
							//self._contextMap.setView([self._focusMap.getCenter().lat, self._focusMap.getCenter().lng], self._contextMap.getZoom() + 1);
							self._contextMap.flyTo([self._focusMap.getCenter().lat, self._focusMap.getCenter().lng], self._contextMap.getZoom() + 1);
							
							// Focus要素を確認
							/*const focusElement = self._focusGlueLayer;
							if (!focusElement) {
								console.error("Error: Focus element (_focusGlueLayer) not found.");
								break;
							}

							const focusRect = focusElement.getBoundingClientRect(); // Focus の位置情報を取得
							const focusCenterX = focusRect.left + focusRect.width / 2; // Focus の中心X座標
							const focusCenterY = focusRect.top + focusRect.height / 2; // Focus の中心Y座標
							...
							try {
								const moveEvent2 = new MouseEvent('mousemove', {
									bubbles: true,
									cancelable: true,
									view: window, // イベント発生元を指定
									clientX: focusCenterX, // Focus の中心にマウスを移動
									clientY: focusCenterY
								});
								focusElement.dispatchEvent(moveEvent2); // Focus にイベントを送信
							} catch (error) {
								console.error("Error creating MouseEvent:", error);
								break;
							}*/
						...
						else if ($("#collision_focus_glue").val() == "changeScale4") { // ContextMapの中心にFocusを固定
							self._contextMap.zoomIn(1);
							break;
						}
					...
					while((Math.pow(e_center.x - center.x,2)+Math.pow(e_center.y - center.y,2) >= Math.pow(radius+e_radius,2)) && (parseInt(e._glueLayer.style.width) < $(e._glueLayer).attr('originalSize'))){//円が衝突してないうちは
						...
						/*else if ($("#collision_focus_glue").val() == "changeScale"){
							if(parseInt(self._glueLayer.style.width) < $(self._glueLayer).attr('originalSize')){
								g_dragDropEvent.changeGlueFocusRadiusRatio($(self._focusLayer), $(self._glueLayer),1);	// focus,glueの大きさを変える.
							}
							else if(parseInt(e._glueLayer.style.width) < $(e._glueLayer).attr('originalSize')){
								...
								// カーソル位置を地図の中心に設定
								var cursorLatLng = self._contextMap.containerPointToLatLng(cursorPosition);
								self._contextMap.setView(cursorLatLng); // カーソル位置を地図の中心に
								self._contextMap.zoomIn(1);
								var moveEvent = new MouseEvent('mousedown', {
									bubbles: true,
									cancelable: true,								
	...
	onAdd: function (map) {
		...
		g_GlobalStaticNumber.polyline.eachLayer(function (polyline) {
			//   g_drawMap.drawPolyline(self._focusMap, self._contextMap, self._focusGlueLayer, self._focusLayer, self._glueLayer, layer);
		...
		//常に実行されるやつ
		/*if ($("#collision_focus_glue").val() == "changeScale4") { // ContextMapの中心にFocusを固定
			let focusPoint = self._contextMap.getCenter(); // 初期状態でのContextMapの中心点を取得
			//console.log("FP:" + focusPoint);
			//let focusP = self._contextMap.latLngToContainerPoint(focusPoint);
			let isUpdating = false; // イベントの再入を防ぐフラグ

			console.log(g_GlobalStaticNumber.layer);
			
			document.body.appendChild(self._focusGlueLayer);
			document.body.appendChild(self._glueLayer);
			document.body.appendChild(self._focusLayer);
			// ウィンドウのリサイズイベントや初期ロード時に中央位置を計算する関数
			function updateLayerToCenter() {
				// ウィンドウの幅と高さを取得
				const windowWidth = window.innerWidth;
				const windowHeight = window.innerHeight+50;

				// レイヤーを中央に配置
				const focusLayerWidth = parseInt(window.getComputedStyle(self._focusGlueLayer).width, 10);
				const focusLayerHeight = parseInt(window.getComputedStyle(self._focusGlueLayer).height, 10);

				self._focusGlueLayer.style.position = "fixed"; // 固定配置
				self._focusGlueLayer.style.left = `${(windowWidth - focusLayerWidth) / 2}px`;
				self._focusGlueLayer.style.top = `${(windowHeight - focusLayerHeight) / 2}px`;

				const glueLayerWidth = parseInt(window.getComputedStyle(self._glueLayer).width, 10);
				const glueLayerHeight = parseInt(window.getComputedStyle(self._glueLayer).height, 10);

				self._glueLayer.style.position = "fixed"; // 固定配置
				self._glueLayer.style.left = `${(windowWidth - glueLayerWidth) / 2}px`;
				self._glueLayer.style.top = `${(windowHeight - glueLayerHeight) / 2}px`;

				const focusLayerWidth2 = parseInt(window.getComputedStyle(self._focusLayer).width, 10);
				const focusLayerHeight2 = parseInt(window.getComputedStyle(self._focusLayer).height, 10);

				self._focusLayer.style.position = "fixed"; // 固定配置
				self._focusLayer.style.left = `${(windowWidth - focusLayerWidth2) / 2}px`;
				self._focusLayer.style.top = `${(windowHeight - focusLayerHeight2) / 2}px`;

				console.log("Layers positioned at center of the page.");
			}

			// ウィンドウリサイズイベントに対応
			window.addEventListener("resize", updateLayerToCenter);

			// 初期配置
			updateLayerToCenter();

			// マップの移動イベントを監視
			self._contextMap.on("moveend", function () {
				// マップが動いたら中心点を元のfocusPointに戻す
				//contextMap.setCenter(focusPoint);
				//self._contextMap.setView(focusPoint, self._contextMap.getZoom());
				if (!isUpdating) { // 再入を防ぐ
					isUpdating = true;
					focusPoint = self._contextMap.getCenter();
					console.log("Updated FP:", focusPoint);
					self._contextMap.setView(focusPoint, self._contextMap.getZoom());
					//focusP = self._contextMap.latLngToContainerPoint(focusPoint);
					//console.log("convert FP:", focusP);
					//focusP.x += 10;
					//focusP.y += 10;

					// ContextMapのコンテナのサイズを取得
					/*const contextMapContainer = self._contextMap.getContainer(); // ContextMapのコンテナを取得
					// コンテナの幅と高さを計算
					const containerWidth = contextMapContainer.clientWidth/2;//1997
					const containerHeight = contextMapContainer.clientHeight/2;//1258
					console.log(containerWidth+"///"+containerHeight);
					// ContextMapの枠内の中央位置
					self._focusGlueLayer.style.left = `${containerWidth - parseInt(self._focusGlueLayer.style.width, 10) / 2}`;
					//self._focusGlueLayer.style.left = "200px";
					self._focusGlueLayer.style.top = `${containerHeight - parseInt(self._focusGlueLayer.style.height, 10) / 2}`;
					//self._focusGlueLayer.style.top = containerHeight;
					//self._glueLayer.style.left = `${containerWidth - parseInt(self._glueLayer.style.width, 10) / 2}`;
					self._glueLayer.style.left = "200";
					//self._glueLayer.style.top = `${containerHeight - parseInt(self._glueLayer.style.height, 10) / 2}`;
					self._glueLayer.style.top = "200";
					self._focusLayer.style.left = `${containerWidth - parseInt(self._focusLayer.style.width, 10) / 2}`;
					self._focusLayer.style.top = `${containerHeight - parseInt(self._focusLayer.style.height, 10) / 2}`;
					console.log(self._focusGlueLayer.style.left+"/fg/"+self._focusGlueLayer.style.top);
					console.log(self._glueLayer.style.left+"/g/"+self._glueLayer.style.top);
					console.log(self._focusLayer.style.left+"/f/"+self._focusLayer.style.top);*/
					//self._focusGlueLayer.style.left = focusP.x-self._focusGlueLayer.style.width/2;
					//self._focusGlueLayer.style.top = focusP.y-self._focusGlueLayer.style.width/2;
					//self._focusGlueLayer.style.left = `${focusP.x - parseInt(self._focusGlueLayer.style.width, 10) / 2}px`;
					//self._focusGlueLayer.style.top = `${focusP.y - parseInt(self._focusGlueLayer.style.height, 10) / 2}px`;
/*					self._focusMap.setView(focusPoint, self._focusMap.getZoom());
					//updateFocusLayerPosition(focusPoint);
					isUpdating = false;
				}
			});

			// ズームレベルを変更した場合も中心を維持する
			self._contextMap.on("zoomend", function () {
				//contextMap.setCenter(focusPoint);
				//self._contextMap.setView(focusPoint, self._contextMap.getZoom());
				if (!isUpdating) { // 再入を防ぐ
					isUpdating = true;
			...
			// FocusLayerの位置を更新する関数
			/*function updateFocusLayerPosition(focusPoint) {
				// FocusLayerの中心をFocusPointに設定
				const layerWidth = parseInt(self._focusGlueLayer.style.width, 10);
				const layerHeight = parseInt(self._focusGlueLayer.style.height, 10);
		
				self._focusGlueLayer.style.left = `${focusPoint.lng - layerWidth / 2}px`;
				self._focusGlueLayer.style.top = `${focusPoint.lat - layerHeight / 2}px`;
		
				console.log("FocusLayer position updated:", self._focusGlueLayer.style.left, self._focusGlueLayer.style.top);
			}
			function updateFocusLayerPosition(focusPoint) {
				// 地図座標 (緯度経度) をピクセル座標に変換
				const pixelCenter = self._contextMap.project(focusPoint);
		
				// FocusLayerの中心を設定
				const layerElement = self._focusGlueLayer;
				const layerWidth = parseFloat(window.getComputedStyle(layerElement).width);
				const layerHeight = parseFloat(window.getComputedStyle(layerElement).height);
		
				// Layerの位置を計算
				const leftPosition = pixelCenter.x - layerWidth / 2;
				const topPosition = pixelCenter.y - layerHeight / 2;
		
				// FocusLayerのスタイルを設定
				layerElement.style.position = 'absolute'; // positionを明示的に指定
				layerElement.style.left = `${leftPosition}px`;
				layerElement.style.top = `${topPosition}px`;
		
				console.log("FocusLayer position updated:", {
					left: layerElement.style.left,
					top: layerElement.style.top,
					focusPoint: focusPoint,
				});
			}*/
/*		} else {
			// イベント解除
			self._contextMap.off("moveend");
		}/*
		if ($("#collision_focus_glue").val() == "changeScale4") {
			let focusPoint = self._contextMap.getCenter(); // 初期状態でのContextMapの中心点を取得
			console.log("Initial FP:", focusPoint);
			let isUpdating = false; // イベントの再入を防ぐフラグ
		
			// マップの移動終了イベントを監視
			self._contextMap.on("moveend", function () {
				// ユーザーが移動操作をした後の中心点を更新
				if (!isUpdating) { // 再入を防ぐ
					focusPoint = self._contextMap.getCenter();
					console.log("Updated FP:", focusPoint);
				//if (!isUpdating) { // 再入を防ぐ
			...
			self._contextMap.on("zoomend", function () {
				focusPoint = self._contextMap.getCenter();
				console.log("Zoom Updated FP:", focusPoint);
				if (!isUpdating) { // 再入を防ぐ
					isUpdating = true;
					self._contextMap.setView(focusPoint, self._contextMap.getZoom());
					isUpdating = false;
		...
		this._contextMap.whenReady(function () {
			...
			createItem.onclick = function() {
				...
				/*create = true;
				// 生成の横に他のオプションを表示
				markerItem.style.display = 'inline-block';
				focusItem.style.display = 'inline-block';
				fixedFocusItem.style.display = 'inline-block';
				//customContextMenu2.style.display = 'none';
				console.log("a");
			...
			markerItem.innerText = 'マーカー';
			markerItem.style.padding = '8px 12px';
			markerItem.style.cursor = 'pointer';
			markerItem.onmouseover = function() { this.style.backgroundColor = '#eee'; };
			markerItem.onmouseout = function() { this.style.backgroundColor = 'white'; };
			...
			focusItem.innerText = '可動Focus';
			focusItem.style.padding = '8px 12px';
			focusItem.style.cursor = 'pointer';
			focusItem.onmouseover = function() { this.style.backgroundColor = '#eee'; };
			focusItem.onmouseout = function() { this.style.backgroundColor = 'white'; };
			focusItem.onclick = function() {
				//右クリックした位置に生成
				console.log("c");
				customContextMenu2.style.display = 'none';
			};
			const fixedFocusItem = document.createElement('li');
			fixedFocusItem.innerText = '固定Focus';
			fixedFocusItem.style.padding = '8px 12px';
			fixedFocusItem.style.cursor = 'pointer';
			fixedFocusItem.onmouseover = function() { this.style.backgroundColor = '#eee'; };
			fixedFocusItem.onmouseout = function() { this.style.backgroundColor = 'white'; };
			fixedFocusItem.onclick = function() {
				//右クリックした位置に生成
				console.log("d");
				customContextMenu2.style.display = 'none';
			};*/
			...
			//menuList2.appendChild(markerItem);
			//menuList2.appendChild(focusItem);
			//menuList2.appendChild(fixedFocusItem);
			...
			this._contextMap.addEventListener('contextmenu', function(event) {
				...
				if (event.originalEvent) {
					...
					fixedFocusItem.style.display = 'none';*/
				} else {
					console.error("originalEventが見つかりません");
				}
			});
			/*this._contextMap.addEventListener('contextmenu', function(event) {
				console.log(event);  // 追加: イベントが正しく発火しているか確認
		
				if (event.originalEvent) {
					event.originalEvent.preventDefault(); // Leafletのイベントオブジェクトから標準のイベントにアクセス
					if (create == true) {
						markerItem.style.display = 'block';
						focusItem.style.display = 'block';
						fixedFocusItem.style.display = 'block';
					} else {
						markerItem.style.display = 'none';
						focusItem.style.display = 'none';
						fixedFocusItem.style.display = 'none';
...
/*		if(init){
			// 複数のマーカーを設定
			var markers = [
				{ lat: 35.170697, lng: 136.881626, comment: "名古屋駅" },
				{ lat: 35.183685, lng: 136.933103, comment: "徳川美術館" },
				{ lat: 35.15500, lng: 136.92003, comment: "鶴舞公園" },
				{ lat: 35.170842, lng: 136.909524, comment: "オアシス21" },
				{ lat: 35.185579, lng: 136.899087, comment: "名古屋城" },
				{ lat: 35.158361, lng: 136.931298, comment: "吹上公園" },
				{ lat: 35.158236, lng: 136.904602, comment: "大須商店街" },
				{ lat: 35.164889, lng: 136.899701, comment: "名古屋市科学館" },
				{ lat: 35.156311, lng: 136.923079, comment: "名工大" },
				{ lat: 35.167373, lng: 136.908826, comment: "久屋大通公園" },
				{ lat: 35.179517, lng: 136.881296, comment: "ノリタケの森" },
				{ lat: 35.180199, lng: 136.905803, comment: "愛知県庁舎" },
				{ lat: 35.178511, lng: 136.895176, comment: "愛知県図書館" }
				//{ lat: 35.125411, lng: 136.909271, comment: "熱田神宮" }
			];
...
function updateFocus() {
    const center = self._contextMap.getCenter(); // ContextMap の中心座標を取得
    focus.setView(center, focusZoomLevel); // Focus を中心に合わせて更新
}

// イベントリスナーでリアルタイム更新
self._contextMap.on('mousemove', updateFocus, self);
self._contextMap.on('zoom', updateFocus);*/
